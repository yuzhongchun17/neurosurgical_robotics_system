function ybar=ybar_calc4(g,do,t,g_tol)
...
if g >= do-t && g<do   % We're cutting down to or past the inner wall
    ...
    ybar_out=(2*r_out/3)*(sin(alpha_out)^3)/(alpha_out-sin(alpha_out)*cos(alpha_out));
...
elseif g>= r_out && g<do-t  % We're cutting down to between the radius and inner wall  
    ...
    ybar_in=(2*r_in/3)*(sin(alpha_in)^3)/(alpha_in-sin(alpha_in)*cos(alpha_in));
...
elseif g>t  && g<r_out  % we're cutting out less than the radius, so we need to subtract out the segment from the tube
    ...
    ybar=ybar_seg*A_seg/(A_tube-A_seg); %b/c ybar of the tube is zero-- centered about is origin
