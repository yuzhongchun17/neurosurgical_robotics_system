function shape=fwd_kin7_Tip(tube_geo,params,E,the_options)
...
options=optimoptions('fsolve','Algorithm','levenberg-marquardt','MaxFunctionEvaluations',90*10^3,'MaxIterations',100000,'FunctionTolerance',1e-45,'StepTolerance',1e-45);%,'Display','iter');
...
%%{
if q<=-h(1)	% if it's past the first notch
	ko_guess=q*alpha(2:end)/sum_factors;
	ki_guess=ko_guess(end)/(1-ko_guess(end)*(d(end)));
	tau_guess=(E/d(end))*(Io(end)*ko_guess(end)+Ii(end)*ki_guess);
	act_var_guess=[ko_guess, tau_guess];
% 	find_act_var=@(act_var)([(q-calc_q(h,d,[0,act_var(1:n-1)]))^2, ([0,act_var(end)*ones(1,n-1)]-calc_tau(E,Io,Ii,d,[0,act_var(1:n-1)])).^2]);
	find_act_var=@(act_var)([(q-calc_q(h,d,[0,act_var(1:n-1)])), ([0,act_var(end)*ones(1,n-1)]-calc_tau(E,Io,Ii,d,[0,act_var(1:n-1)]))]);
	try
		act_var=fsolve(find_act_var,act_var_guess,options);
	catch
		disp('DANGER, WILL ROBINSON. Catastrophic error in your code!')
	end
	kappa_o=[0,act_var(1:n-1)];
elseif q<=-(h(1)+c(1)+h(2))	% if it's past the second notch
	ko_guess=q*alpha(3:end)/sum_factors;
	ki_guess=ko_guess(end)/(1-ko_guess(end)*(d(end)));
	tau_guess=(E/d(end))*(Io(end)*ko_guess(end)+Ii(end)*ki_guess);
	act_var_guess=[ko_guess, tau_guess];
% 	find_act_var=@(act_var)([(q-calc_q(h,d,[0,0,act_var(1:n-2)]))^2, ([0,0,act_var(end)*ones(1,n-2)]-calc_tau(E,Io,Ii,d,[0,0,act_var(1:n-2)])).^2]);
	find_act_var=@(act_var)([(q-calc_q(h,d,[0,0,act_var(1:n-2)])), ([0,0,act_var(end)*ones(1,n-2)]-calc_tau(E,Io,Ii,d,[0,0,act_var(1:n-2)]))]);
	act_var=fsolve(find_act_var,act_var_guess,options);
	kappa_o=[0,0,act_var(1:n-2)];
else
	ko_guess=q*alpha/sum_factors;
	ki_guess=ko_guess(end)/(1-ko_guess(end)*(d(end)));
	tau_guess=(E/d(end))*(Io(end)*ko_guess(end)+Ii(end)*ki_guess);
	act_var_guess=[ko_guess, tau_guess];
% 	find_act_var=@(act_var)([(q-calc_q(h,d,act_var(1:n)))^2, (act_var(end)*ones(1,n)-calc_tau(E,Io,Ii,d,act_var(1:n))).^2]);
	find_act_var=@(act_var)([(q-calc_q(h,d,act_var(1:n))), (act_var(end)*ones(1,n)-calc_tau(E,Io,Ii,d,act_var(1:n)))]);
	act_var=fsolve(find_act_var,act_var_guess,options);
	kappa_o=act_var(1:n);%ko_guess;%
end
