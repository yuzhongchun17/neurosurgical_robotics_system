...
%% Desired Parameters
curve_option=1;
x_des=30; y_des=20; z_des=30;

t_phi=pi/4; t_theta=pi/4; t_psi=pi/2;

%% Get Transformation Matrices
T_base=eye(4);	% assume we're starting out at zero zero zero, but this can change.

p_des=[x_des; y_des; z_des];
% This is a fixed-frame rotation-- requires premultiplication. These are
% all angles from the base frame axes.
R_des=[	cos(t_phi)*cos(t_theta),	cos(t_phi)*sin(t_theta)*sin(t_psi)-sin(t_phi)*cos(t_psi),	cos(t_phi)*sin(t_theta)*cos(t_psi)+sin(t_phi)*sin(t_psi);
		sin(t_phi)*cos(t_theta),	sin(t_phi)*sin(t_theta)*sin(t_psi)+cos(t_phi)*cos(t_psi),	sin(t_phi)*sin(t_theta)*cos(t_psi)-cos(t_phi)*sin(t_psi);
		-sin(t_theta),				cos(t_theta)*sin(t_psi),									cos(t_theta)*cos(t_psi)];

T_tip=[[R_des; 0, 0, 0],[p_des; 1]];
...
phi=atan2(PP(2),PP(1));
N_PP=[-sin(theta), 0, cos(theta)].';	% needs to change for 3D
L_guess=0.25*L_PP;
% change this to some relationship between tip angle and angle of PP
% if theta<pi/6
	P2g=0.5*PP+0.5*L_PP*N_PP;
% else
...
% 	end
% end
init_guess=[L_guess,L_guess,P2g(1),P2g(2),P2g(3)];
...
%%{
p2_bound=1;
LB=[0.1*L_PP, 0.1*L_PP, P2g(1)-p2_bound*L_PP,P2g(2)-p2_bound*L_PP,P2g(3)-p2_bound*L_PP]; UB=[L_PP, L_PP, P2g(1)+p2_bound*L_PP,P2g(2)+p2_bound*L_PP,P2g(3)+p2_bound*L_PP];
[otps,fvale]=fmincon(@(the_params)bez_curvature3D(T_base,T_tip,the_params(1),the_params(2),the_params(3),the_params(4),the_params(5),curve_option),init_guess,[],[],[],[],LB,UB);
%}
L1=otps(1); L3=otps(2); P2=[otps(3), otps(4), otps(5)].';
...
% This is for plotting the desired poses
tip_tan_start=P4;
tip_tan_end=tip_tan_start+1.5*V4;
base_tan_start=P0;
base_tan_end=base_tan_start+1.5*V0;
...
figure
plot3(xB,yB,zB,'k')	% the curve
hold on; grid on; daspect([1 1 1])
plot3(xB0,yB0,zB0,'r')	% the curve
plot3(le_points(1,:),le_points(2,:),le_points(3,:),'k--x')	% the optimized control polygon
plot3(le_points_guess(1,:),le_points_guess(2,:),le_points_guess(3,:),'r--x')	% the original control polygon
% legend('Curve','Optimize/d Control Polygon','Initial Control Polygon')
plot3(tip_normal(1,:),tip_normal(2,:),tip_normal(3,:),'b')	% the tip normal (to help it be clear on the plot)
plot3(base_normal(1,:),base_normal(2,:),base_normal(3,:),'b')	% the tip normal (to help it be clear on the plot)
...
% if we want to compare this case to a circle
if tip_angle==pi/2 && x_des==z_des
	i=0;
	for t=pi/2:pi/32:pi
		i=i+1;
		circ_x(i)=x_des+z_des*cos(t); circ_y(i)=z_des*sin(t);
	end
...
% subplot(1,3,1)
	...
	hold on; grid on; daspect([1 1 1])
	plot([0.5*P4(1),P2g(1)],[0.5*P4(3),P2g(3)],'k')
	plot([P0(1),P1g(1)],[P0(3),P1g(3)],'k')
	plot([P4(1),P3g(1)],[P4(3),P3g(3)],'k')
...
% 	plot(tip_normal(1,:),tip_normal(2,:),'b')	% the tip normal (to help it be clear on the plot)
% 	plot(base_normal(1,:),base_normal(2,:),'b')	% the tip normal (to help it be clear on the plot)
% 	arrow(tip_tan_start,tip_tan_end,'Length',10,'Width',1,'BaseAngle',90,'TipAngle',15)		% tip tangent
% 	arrow(base_tan_start,base_tan_end,'Length',10,'Width',1,'BaseAngle',90,'TipAngle',15)	% base tangent
% subplot(1,3,3)
